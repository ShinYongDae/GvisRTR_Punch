...
void CMpDevice::GetMpeIO()
{
	...
	for (nLoop = 0; nLoop < (nGrpIn+nGrpOut); nLoop++)
	{
		...
		for (nIdx = 0; nIdx < nGrpStep; nIdx++)
		{
			...
			m_pMpeIo[nIdx + nSt] = (unsigned short)RegWData;
		}
		nSt += nGrpStep;
		Sleep(10);
	}
...
void CMpDevice::GetMpeSignal()
{
	...
	for (nLoop = 0; nLoop < nGrpIn; nLoop++)
	{
		...
		{
			RegWData = *((WORD*)RegInfo[nLoop].pRegisterData + nIdx);
			m_pMpeIb[nIdx + nSt] = (unsigned short)RegWData;
		}
		nSt += nGrpStep;
	}
	...
	{
		RegWData = *((WORD*)RegInfo[nLoop].pRegisterData + nIdx);
		m_pMpeSignal[nIdx + nSt] = (unsigned short)RegWData;
	}
	...
	for (nIdx = 0; nIdx < nGrpStep; nIdx++)
	{
		for (k = 0; k < 16; k++)
		{
			...
			m_pMpeData[nIdx + nSt][k] = (long)RegLData;
		}
	}
...
void CMpDevice::InitIO()
{
	int i, k;

#ifdef USE_MPE
	m_nMpeSignal = pDoc->MkIo.MpeSignal.nMaxSeg;
	if (!m_pMpeSignal)
	{
		if (m_nMpeSignal > 0)
		{
			m_pMpeSignal = new unsigned short[m_nMpeSignal];
			for (i = 0; i < m_nMpeSignal; i++)
				m_pMpeSignal[i] = 0;
		}
	}


	m_nMpeDataMaxSeg = pDoc->MkIo.MpeData.nMaxSeg;
	if (!m_pMpeData)
	{
		if (m_nMpeDataMaxSeg > 0)
		{
			m_pMpeData = new long*[m_nMpeDataMaxSeg];
			for (i = 0; i < m_nMpeDataMaxSeg; i++)
			{
				m_pMpeData[i] = new long[16];
				for (k = 0; k < 16; k++)
					m_pMpeData[i][k] = 0;
			}
		}
...
char* CMpDevice::StringToChar(CString str) // char* returned must be deleted... 
{
	char*		szStr = NULL;
	wchar_t*	wszStr;
	int				nLenth;
