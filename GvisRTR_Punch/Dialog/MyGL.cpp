...
{
	int i, k, nDef;
	if (!m_pReelMap || !m_pFrmRgn || !m_pPcsPnt)
		return;

	for (k = m_nTotPnl - 1; k >= 0; k--)
	{
		//if (m_pPnlNum[k] > pDoc->m_nLotLastShot)
		//	return;

		if (k == m_pReelMap->m_nSelMarkingPnl)
		{
			GVGLDrawInit(GV_LINE, 2, m_rgbRed);
			GVGLDrawRectF(m_pFrmRgn[k][0], m_pFrmRgn[k][1]);
			GVGLDrawShow();
		}
		else if (k == m_pReelMap->m_nSelMarkingPnl + 1)
		{
			GVGLDrawInit(GV_LINE, 2, m_rgbRed);
			GVGLDrawRectF(m_pFrmRgn[k][0], m_pFrmRgn[k][1]);
			GVGLDrawShow();
...
tpOs CMyGL::GetWindowVersion()
{
	return m_cWindowVersionCollect.GetOsInfo();
}

void CMyGL::SaveLog(CString strMsg)
{
	...
	*pszPos = NULL;
	...
	_stprintf(szFile, _T("%s\\%s_MyGL.txt"), szPath, COleDateTime::GetCurrentTime().Format(_T("%Y%m%d")));
